...
def f_score(inputs, target, smooth=1e-5, threhold=0.5):
    n, c, h, w = inputs.size()
    ...
    temp_inputs = torch.sigmoid(inputs.squeeze(1).contiguous().view(n, -1)) * (
        1 - target[..., -1].view(n, -1)
    )
    temp_inputs = torch.gt(temp_inputs, threhold).float()
    temp_target = target[..., 1].view(n, -1)
    tp = torch.sum(temp_target * temp_inputs, axis=1)
    fn = torch.sum(temp_target, axis=1)
    fp = torch.sum(temp_inputs, axis=1)
    score = (2.0 * tp + smooth) / (fn + fp + smooth)
...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    # Create a confusion matrix with all zeros
    ...
    # Reads each (image-tag) pair
    for ind in range(len(gt_imgs)):
        # Read an image segmentation result, transform into numpy array
        pred = np.array(Image.open(pred_imgs[ind]))
        # Read a sheet of corresponding labels into a numpy array
        label = np.array(Image.open(gt_imgs[ind]))

        # If the image segmentation result is not the same size as the label, this image is not calculated
        ...
        # Compute a 21×21 hist matrix for an image and accumulate the
        hist += fast_hist(label.flatten(), pred.flatten(), num_classes)
        # Output the mIoU value averaged over all categories for every 10 images that have been computed so far
    ...
    # Find the mIoU value averaged over all categories on all validation set images, ignoring NaN values in the calculation
    print(
        "===> mIoU: "
